// app/units/page.tsx — Unidades del LearningPath activo.
// Fase 1: el primer path disponible (típicamente Math · Primary 1).
import { redirect } from "next/navigation";
import Link from "next/link";
import { getActiveChild, getDefaultLearningPath, getUnitsWithProgress } from "@/lib/queries";
        ...
        <div className="max-w-6xl mx-auto px-4 md:px-8 py-6 md:py-10">
          <div className="mb-4 md:mb-6">
            <div className="text-[10px] font-extrabold text-ink-mute tracking-widest">
              {path.subject.name.toUpperCase()} · {path.name.toUpperCase()}
            </div>
          ...
          {units.map((u, i) => {
            ...
            return (
              <Link
                key={u.id}
                href={`/home?unit=${u.slug}`}
                className={`btn-chunky flex md:flex-col md:items-start items-center gap-3 md:gap-4 p-4 md:p-6 rounded-3xl border-4 border-white bg-gradient-to-br ${bg}`}
                style={{ boxShadow: "var(--shadow-chunky)" }}
              >
                <div className="w-14 h-14 md:w-16 md:h-16 rounded-2xl flex items-center justify-center text-3xl md:text-4xl border-2 border-white flex-shrink-0 bg-white/85">
                  {u.icon ?? "📚"}
                </div>
                ...
                <div className="md:self-end">
                  {done
                    ? <div className="w-8 h-8 rounded-full bg-mint flex items-center justify-center text-white font-bold" style={{ boxShadow: "0 2px 0 #4DA86A" }}>✓</div>
                    : <div className="text-[10px] font-black bg-ink text-white px-3 py-1.5 rounded-lg">JUGAR</div>}
                </div>
              </Link>
            );
          })}
