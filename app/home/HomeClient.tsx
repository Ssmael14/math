...
export function HomeClient({
  ...
  child: { hearts: number; gems: number; streak: number };
  ...
  return (
    <div className="min-h-[100dvh] flex flex-col bg-gradient-to-b from-sky-soft via-cream to-mint-soft md:bg-none md:bg-white">
      {/* TOP BAR */}
      <header className="md:sticky md:top-0 md:z-20 md:bg-white md:border-b md:border-ink/5">
        <div className="max-w-6xl mx-auto px-4 md:px-8 pt-12 md:pt-0 md:h-16 flex items-center justify-between gap-3">
          <div className="hidden md:block font-fredoka text-xl font-bold text-ink">🦙 LearnMath</div>
          <div className="flex items-center gap-2 md:ml-auto">
            <Stat icon="🔥" value={child.streak}/>
            <Stat icon="💎" value={child.gems}/>
            <Stat icon="❤️" value={child.hearts}/>
          </div>
        </div>
      </header>

      {/* CONTENIDO centrado verticalmente en desktop */}
      <main className="flex-1 w-full flex flex-col md:justify-center">
        <div className="w-full max-w-6xl mx-auto px-4 md:px-8 pb-28 md:pb-12 md:py-16 md:grid md:grid-cols-[360px_1fr] md:gap-16">
...
function Stat({ icon, value }: { icon: string; value: number }) {
  return (
    <div className="flex items-center gap-1 bg-white/90 md:bg-cream backdrop-blur rounded-full px-3 py-1.5">
      <span>{icon}</span>
      <span className="font-black text-sm text-ink">{value}</span>
    </div>
  );
}
